...
type Handler interface {
	...
	InitTrasaAuth(c *Conn, salt []byte, user string,  clearPassword string) error
}
...
func (l *Listener) handle(conn net.Conn, connectionID uint32, acceptTime time.Time) {
	...
	default:
		...
		// auth server.
		clearPassword, err = AuthServerNegotiateClearOrDialog(c, authServerMethod)
	...
	err=l.handler.InitTrasaAuth(c,salt,user,clearPassword)
	if err != nil {
		log.Errorf("Could not authenticate with trasa: %v",err)
		c.writeErrorPacketFromError(err)
		return
	}
